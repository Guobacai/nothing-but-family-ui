...
import { useCreateRecordMutation } from '../../api/app';
...
function SpendingForm({ onClickCancel, year, month }: SpendingFormProps) {
  ...
  const onSubmit: SubmitHandler<FormData> = async (data) => {
    ...
    const {
      ...
      isSuccess,
      isError,
    } = await createRecordRequest({ items: data, year, month });
    ...
    <form onSubmit={handleSubmit(onSubmit)}>
      <Fieldset className="mm-flex mm-flex-col mm-gap-5">
        ...
        <Field className="mm-flex mm-flex-col">
          ...
          <Input
            className="mm-border-2 mm-border-black mm-rounded"
            type="number"
            {...register('totalSalary', { valueAsNumber: true })}
          />
        </Field>
        <Field className="mm-flex mm-flex-col">
          ...
          <Input
            className="mm-border-2 mm-border-black mm-rounded"
            type="number"
            {...register('aptCommonCharge', { valueAsNumber: true })}
          />
        </Field>
        <Field className="mm-flex mm-flex-col">
          ...
          <Input
            className="mm-border-2 mm-border-black mm-rounded"
            type="number"
            {...register('utility', { valueAsNumber: true })}
          />
        </Field>
        <Field className="mm-flex mm-flex-col">
          ...
          <Input
            className="mm-border-2 mm-border-black mm-rounded"
            type="number"
            {...register('aptMortgage', { valueAsNumber: true })}
          />
        </Field>
        <Field className="mm-flex mm-flex-col">
          ...
          <Input
            className="mm-border-2 mm-border-black mm-rounded"
            type="number"
            {...register('merrill', { valueAsNumber: true })}
          />
        </Field>
        <Field className="mm-flex mm-flex-col">
      ...
      <div className="mm-flex mm-justify-evenly">
        <Button
          className="mm-bg-neutral-200 mm-p-2 mm-rounded-lg"
